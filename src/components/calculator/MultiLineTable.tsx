...
export function MultiLineTable({ result }: MultiLineTableProps) {
  const { productLines, horizonYears } = result;
    ...
    <Card className="print:break-inside-avoid">
      <CardHeader>
        ...
        <CardDescription>
          ...
          multi-year new production + renewals. Eligibility uses your Medicare-age / under-65 mix.
        </CardDescription>
      </CardHeader>
          ...
          {productLines.map((line) => (
            <div
              ...
              className={cn(
                ...
                line.isOffered ? "border-border bg-muted/20" : "border-primary/15 bg-primary/[0.02]",
              )}
            >
              ...
              <dl className="mt-3 grid grid-cols-2 gap-2 text-xs">
                ...
                <div>
                  <dt className="text-muted-foreground">Renewal $ / case</dt>
                  <dd className="tabular-nums">{formatCurrency(line.renewalRevenue)}</dd>
                </div>
              </dl>
            </div>
          ))}
